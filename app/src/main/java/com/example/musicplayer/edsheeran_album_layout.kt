...
import com.example.musicplayer.databinding.ActivityEdsheeranAlbumLayoutBinding
import com.example.musicplayer.model.Music
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.io.File

class edsheeran_album_layout : AppCompatActivity() {
    private lateinit var musicAdapter: MusicAdapter
    private val binding: ActivityEdsheeranAlbumLayoutBinding by lazy {
        ActivityEdsheeranAlbumLayoutBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageButton11.setOnClickListener{
            ...
            intent.putExtra("class", "edsheeran_album_layout")
            startActivity(intent)
        }


        // Copy assets only if the "Ed Sheeran" folder doesn't exist
        if (!isESFolderExists()) {
            downloadFolder()
        }

        // Get songs after ensuring the "Ed Sheeran" folder exists
        musicListMA = getallaudioo()

        // Initialize musicAdapter before calling notifyDataSetChanged
        musicAdapter = MusicAdapter(this@edsheeran_album_layout, musicListMA)

        // Set up RecyclerView
        binding.musicRVinAlbum.setHasFixedSize(true)
        binding.musicRVinAlbum.setItemViewCacheSize(20)
        binding.musicRVinAlbum.layoutManager = LinearLayoutManager(this@edsheeran_album_layout)
    ...
    //List All The Songs From Ed Sheeran Folder To RecyclerView
    //Start
    @SuppressLint("Range")
    private fun getallaudioo(): ArrayList<Music> {
        ...
        val musicDirPath = "${Environment.DIRECTORY_DOWNLOADS}/Ed Sheeran"
    ...
    private fun isESFolderExists(): Boolean {
        val musicDir = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)}/Ed Sheeran")
...
/* private fun copyAssets(context: Context) {
        ...
        val files = assetManager.list("Ed Sheeran")
        val downloadDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
        val musicDir = File(downloadDir, "Ed Sheeran")
        ...
        if (files != null) {
            for (filename in files) {
                try {
                    val asset = assetManager.open("Ed Sheeran/$filename")
            ...
            Log.e("CopyAssets", "No files found in assets/Ed Sheeran")
        }
    }*//*
    ...
    fun downloadFolder() {
        val downloadDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
        val loFiFolder = File(downloadDir, "Ed Sheeran")

        if (!loFiFolder.exists()) {
            loFiFolder.mkdirs()
        }

        val loFiStorageRef: StorageReference = storageReference.child("Ed Sheeran")
...
class edsheeran_album_layout : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.musicRVinAlbum.apply {
            ...
            layoutManager = LinearLayoutManager(this@edsheeran_album_layout)
    ...
    private fun retrieveMusicData() {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("song/Ed Sheeran")
