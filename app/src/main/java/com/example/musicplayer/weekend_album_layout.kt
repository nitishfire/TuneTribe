...
import com.example.musicplayer.databinding.ActivityWeekendAlbumLayoutBinding
import com.example.musicplayer.model.Music
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.io.File

class weekend_album_layout : AppCompatActivity() {
    private lateinit var musicAdapter: MusicAdapter
    private val binding: ActivityWeekendAlbumLayoutBinding by lazy {
        ActivityWeekendAlbumLayoutBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageButton11.setOnClickListener{
            ...
            intent.putExtra("class", "weekend_album_layout")
            startActivity(intent)
        }


        // Copy assets only if the "Weekend" folder doesn't exist
        downloadFolder()

        // Get songs after ensuring the "Weekend" folder exists
        musicListMA = getallaudioo()

        // Initialize musicAdapter before calling notifyDataSetChanged
        musicAdapter = MusicAdapter(this@weekend_album_layout, musicListMA)

        // Set up RecyclerView
        binding.musicRVinAlbum.setHasFixedSize(true)
        binding.musicRVinAlbum.setItemViewCacheSize(20)
        binding.musicRVinAlbum.layoutManager = LinearLayoutManager(this@weekend_album_layout)
    ...
    //List All The Songs From Weekend Folder To RecyclerView
    //Start
    @SuppressLint("Range")
    private fun getallaudioo(): ArrayList<Music> {
        ...
        val musicDirPath = "${Environment.DIRECTORY_DOWNLOADS}/Weekend"
    ...
    private fun isWFolderExists(): Boolean {
        val musicDir = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)}/Weekend")
    ...
    fun downloadFolder() {
        ...
        val loFiFolder = File(downloadDir, "Weekend")

        if (!loFiFolder.exists()) {
            loFiFolder.mkdirs()
        }

        val loFiStorageRef: StorageReference = storageReference.child("Weekend")
...
class weekend_album_layout : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.musicRVinAlbum.apply {
            ...
            layoutManager = LinearLayoutManager(this@weekend_album_layout)
    ...
    private fun retrieveMusicData() {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("song/Weekend")
