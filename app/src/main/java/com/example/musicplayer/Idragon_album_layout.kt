...
import com.example.musicplayer.databinding.ActivityIdragonAlbumLayoutBinding
import com.example.musicplayer.model.Music
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.io.File

class Idragon_album_layout : AppCompatActivity() {
    private lateinit var musicAdapter: MusicAdapter
    private val binding: ActivityIdragonAlbumLayoutBinding by lazy {
        ActivityIdragonAlbumLayoutBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageButton11.setOnClickListener{
            ...
            intent.putExtra("class", "Idragons_album_layout")
            startActivity(intent)
        }


        // Copy assets only if the "Imagine Dragons" folder doesn't exist
        if (!isIDFolderExists()) {
            downloadFolder()
        }

        // Get songs after ensuring the "Imagine Dragons" folder exists
        musicListMA = getallaudioo()

        // Initialize musicAdapter before calling notifyDataSetChanged
        musicAdapter = MusicAdapter(this@Idragon_album_layout, musicListMA)

        // Set up RecyclerView
        binding.musicRVinAlbum.setHasFixedSize(true)
        binding.musicRVinAlbum.setItemViewCacheSize(20)
        binding.musicRVinAlbum.layoutManager = LinearLayoutManager(this@Idragon_album_layout)
    ...
    //List All The Songs From Imagine Dragons Folder To RecyclerView
    //Start
    @SuppressLint("Range")
    private fun getallaudioo(): ArrayList<Music> {
        ...
        val musicDirPath = "${Environment.DIRECTORY_DOWNLOADS}/Imagine Dragons"
    ...
    private fun isIDFolderExists(): Boolean {
        val musicDir = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)}/Imagine Dragons")
   ...
   /* private fun copyAssets(context: Context) {
        ...
        val files = assetManager.list("Imagine Dragons")
        val downloadDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
        val musicDir = File(downloadDir, "Imagine Dragons")
        ...
        if (files != null) {
            for (filename in files) {
                try {
                    val asset = assetManager.open("Imagine Dragons/$filename")
            ...
            Log.e("CopyAssets", "No files found in assets/Imagine Dragons")
        }
    }*/
    ...
    fun downloadFolder() {
        val downloadDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)
        val loFiFolder = File(downloadDir, "Imagine Dragons")

        if (!loFiFolder.exists()) {
            loFiFolder.mkdirs()
        }

        val loFiStorageRef: StorageReference = storageReference.child("Imagine Dragons")
...
class Idragon_album_layout : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageButton11.setOnClickListener{
            val intent  = Intent(this, PlayerActivity::class.java)
            intent.putExtra("index", 0)
            intent.putExtra("class", "Idragon_album_layout")
            startActivity(intent)
        }
        ...
        binding.musicRVinAlbum.apply {
            ...
            layoutManager = LinearLayoutManager(this@Idragon_album_layout)
    ...
    private fun retrieveMusicData() {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("song/Imagine Dragons")
