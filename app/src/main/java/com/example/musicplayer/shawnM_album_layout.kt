...
import com.example.musicplayer.databinding.ActivityShawnMalbumLayoutBinding
import com.example.musicplayer.model.Music
import com.google.firebase.storage.FirebaseStorage
import com.google.firebase.storage.StorageReference
import java.io.File

class shawnM_album_layout : AppCompatActivity() {
    private lateinit var musicAdapter: MusicAdapter
    private val binding: ActivityShawnMalbumLayoutBinding by lazy {
        ActivityShawnMalbumLayoutBinding.inflate(layoutInflater)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.imageButton11.setOnClickListener{
            ...
            intent.putExtra("class", "shawnM_album_layout")
            startActivity(intent)
        }


        // Copy assets only if the "Shawn Mendes" folder doesn't exist
        if (!isSMFolderExists()) {
            downloadFolder()
        }

        // Get songs after ensuring the "Shawn Mendes" folder exists
        musicListMA = getallaudioo()

        // Initialize musicAdapter before calling notifyDataSetChanged
        musicAdapter = MusicAdapter(this@shawnM_album_layout, musicListMA)

        // Set up RecyclerView
        binding.musicRVinAlbum.setHasFixedSize(true)
        binding.musicRVinAlbum.setItemViewCacheSize(20)
        binding.musicRVinAlbum.layoutManager = LinearLayoutManager(this@shawnM_album_layout)
    ...
    //List All The Songs From Shawn Mendes Folder To RecyclerView
    //Start
    @SuppressLint("Range")
    private fun getallaudioo(): ArrayList<Music> {
        ...
        val musicDirPath = "${Environment.DIRECTORY_DOWNLOADS}/Shawn Mendes"
    ...
    private fun isSMFolderExists(): Boolean {
        val musicDir = File("${Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS)}/Shawn Mendes")
    ...
    fun downloadFolder() {
        ...
        val loFiFolder = File(downloadDir, "Shawn Mendes")

        if (!loFiFolder.exists()) {
            loFiFolder.mkdirs()
        }

        val loFiStorageRef: StorageReference = storageReference.child("Shawn Mendes")
...
class shawnM_album_layout : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.musicRVinAlbum.apply {
            ...
            layoutManager = LinearLayoutManager(this@shawnM_album_layout)
    ...
    private fun retrieveMusicData() {
        ...
        val databaseReference = FirebaseDatabase.getInstance().getReference("song/Shawn Mendes")
